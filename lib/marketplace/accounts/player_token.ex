defmodule Marketplace.Accounts.PlayerToken do
  use Ecto.Schema
  import Ecto.Query
  ...
  def build_session_token(player) do
    ...
    {token, %Marketplace.Accounts.PlayerToken{token: token, context: "session", player_id: player.id}}
  end
  ...
  defp build_hashed_token(player, context, sent_to) do
    ...
    {Base.url_encode64(token, padding: false),
     %Marketplace.Accounts.PlayerToken{
       token: hashed_token,
       context: context,
       sent_to: sent_to,
       player_id: player.id
  ...
  def token_and_context_query(token, context) do
    from Marketplace.Accounts.PlayerToken, where: [token: ^token, context: ^context]
  end
  ...
  def player_and_contexts_query(player, :all) do
    from t in Marketplace.Accounts.PlayerToken, where: t.player_id == ^player.id
  end

  def player_and_contexts_query(player, [_ | _] = contexts) do
    from t in Marketplace.Accounts.PlayerToken, where: t.player_id == ^player.id and t.context in ^contexts
  end
end

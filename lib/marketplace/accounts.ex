defmodule Marketplace.Accounts do
  ...
  alias Marketplace.Accounts.{Player, PlayerToken, PlayerNotifier}
  ...
  defp player_email_multi(player, email, context) do
    changeset = player |> Player.email_changeset(%{email: email}) |> Player.confirm_changeset()
  ...
  ## Examples
      ...
      iex> deliver_player_confirmation_instructions(player, &Routes.player_confirmation_url(conn, :confirm, &1))
      {:ok, %{to: ..., body: ...}}

      iex> deliver_player_confirmation_instructions(confirmed_player, &Routes.player_confirmation_url(conn, :confirm, &1))

defmodule MarketplaceWeb.PlayerSessionControllerTest do
  ...
  describe "POST /player/log_in" do
    ...
    test "logs the player in with remember me", %{conn: conn, player: player} do
      ...
      assert conn.resp_cookies["_marketplace_web_player_remember_me"]
      assert redirected_to(conn) =~ "/"
    end
  ...
  describe "DELETE /player/log_out" do

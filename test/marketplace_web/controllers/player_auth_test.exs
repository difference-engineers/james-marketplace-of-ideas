defmodule MarketplaceWeb.PlayerAuthTest do
  use MarketplaceWeb.ConnCase, async: true

  alias Marketplace.Accounts
  alias MarketplaceWeb.PlayerAuth
  import Marketplace.AccountsFixtures

  @remember_me_cookie "_marketplace_web_player_remember_me"

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, MarketplaceWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_player/3" do
    test "stores the player token in the session", %{conn: conn, player: player} do
      ...
      assert get_session(conn, :live_socket_id) == "player_sessions:#{Base.url_encode64(token)}"
  ...
  describe "logout_player/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      live_socket_id = "player_sessions:abcdef-token"
      MarketplaceWeb.Endpoint.subscribe(live_socket_id)
      ...
      assert_receive %Phoenix.Socket.Broadcast{
        event: "disconnect",
        topic: "player_sessions:abcdef-token"
      }
    end

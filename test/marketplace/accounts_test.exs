defmodule Marketplace.AccountsTest do
  ...
  describe "get_player!/1" do
    test "raises if id is invalid" do
      assert_raise Ecto.NoResultsError, fn ->
        Accounts.get_player!("11111111-1111-1111-1111-111111111111")
      end
    end
  ...
  describe "register_player/1" do
    ...
    test "registers player with a hashed password" do
    ...
    test "sends token through notification", %{player: player} do
      token =
        extract_player_token(fn url ->
          Accounts.deliver_update_email_instructions(player, "current@example.com", url)
        end)
  ...
  describe "update_player_email/2" do
    setup do
      ...
      token =
        extract_player_token(fn url ->
          Accounts.deliver_update_email_instructions(%{player | email: email}, player.email, url)
        end)
